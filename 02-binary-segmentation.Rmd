...
has smaller loss. Maybe BinSeg is not allowing segments with a single
data point? (minseglen=1 for both BinSeg and changepoint, so this does
not seem to explain the difference)


```{r}
cum.vec <- cumsum(c(0,one.pid.chr$logratio))
for(k in 2:5){
  end.mat <- rbind(
    manual=c(sort(cp.models.manual@cpts.full[k-1,]), nrow(one.pid.chr)),
    BinSeg=BinSeg::coef(BS.models, k)$end,
    binsegRcpp=coef(bs.models, k)$end)
  print(cbind(end.mat, loss=apply(end.mat, 1, function(end){
    seg.size <- diff(c(0,end))
    seg.mean <- (cum.vec[end+1]-cum.vec[end-seg.size+1])/seg.size
    data.mean <- rep(seg.mean, seg.size)
    sum((data.mean-one.pid.chr$logratio)^2)
  }), segments=k))
}
```

## Another sequence of models

Another way to run it is via the code below, which gives different
results from the methods above, because it uses the MBIC penalty.
...
```{r}
bs.initial <- list(to.split=data.table(start=1L, end=nrow(one.pid.chr)))
```
...
for(n.segments in 2:nrow(one.pid.chr)){
