...
```{r}
cum.data.vec <- cumsum(c(0,one.pid.chr$logratio))
for(k in 2:5){
  end.mat <- rbind(
...
plot.split <- function(it){
  it.vlines <- it$segments[start>1][startChange==FALSE, computed := "previously"]
  computed.colors <- c(
    "this step"="red",
...
## Comparison with wbs::sbs

```{r}
x <- c(0, 0.1, 1, 1.1, 0, 0.1, 1, 1.1)
fit <- wbs::sbs(x)
data.table(fit$res)[order(-min.th, scale)]
```
